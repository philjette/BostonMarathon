...
To avoid erros when selecting years, countries not represented in each year were removed. This was done for simplicity and will be remedied in later versions. 
...
## Shiny UI
The Shiny UI then allows for filtering of the data, and returns some summary statistics by country.
Here is an example Histporam for the results of all countries across all years.
```{r EVAL=TRUE, echo=FALSE}
hist(results$Official.Times, breaks = 40, col = 'darkgray', border = 'white', xlab="Official Time (Minutes)", main="Boston Marathon Finishers")
```
